...
def generateVOC2Json(rootDir,xmlFiles,cocoName,dstDir,startId):
  ...
  attrDict["categories"]=[{"supercategory":"none","id":1,"name":"bordered"}, #Table"},
                          {"supercategory":"none","id":2,"name":"borderless"}
                         ]
  images = list()
  annotations = list()
  id1 = 1
  ...
  for root, dirs, files in os.walk(rootDir):
    ...
    for file in xmlFiles:
      ...
      if file in files:
        ...
        # image['file_name'] = '%d.png'%(image_id) #int
...
#          shutil.copy2(image_path, os.path.join(dstDir, "JPEGImages", "%d.png"%(image_id)))
  ...
  print(len(images))
  print("Bordered : ",cnt_bor," Bless : ",cnt_bless)
...
if __name__ == "__main__":
  ...
  # Start execution
  startId = 0
  endId = generateVOC2Json(rootDir, trainXMLFiles, "../data/VOC2007/trainval.json", destDir, startId)
# int
...
#      f.write("%d\n"%(i))
  ...
  # Start execution
  startId = endId
  endId = generateVOC2Json(rootDir, trainXMLFiles, "../data/VOC2007/test.json", destDir, startId)
# int
